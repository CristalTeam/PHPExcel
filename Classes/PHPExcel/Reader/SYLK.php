...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    protected function isValidFormat()
    {
        ...
        $delimiterCount = substr_count((string) $data, ';');
        if ($delimiterCount < 1) {
            return false;
        }
        ...
        $lines = explode("\n", (string) $data);
        if (!str_starts_with((string) $lines[0], 'ID;P')) {
            return false;
        }
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'P') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'P':
                            $formatArray['numberformat']['code'] = str_replace($fromFormats, $toFormats, substr((string) $rowDatum, 1));
                            break;
                        case 'E':
                        case 'F':
                        ...
                        case 'S':
                            ...
                            for ($i=0; $i<strlen((string) $styleSettings); ++$i) {
