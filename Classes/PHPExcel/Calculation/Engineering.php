...
class PHPExcel_Calculation_Engineering
{
    ...
    public static function parseComplex($complexNumber)
    {
        ...
        $suffix = substr((string) $workString, -1);
        if (!is_numeric($suffix)) {
            $workString = substr((string) $workString, 0, -1);
        } else {
            $suffix = '';
        }
        ...
        $leadingSign = 0;
        if (strlen((string) $workString) > 0) {
            $leadingSign = (($workString[0] == '+') || ($workString[0] == '-')) ? 1 : 0;
        }
        $power = '';
        $realNumber = strtok($workString, '+-');
        if (strtoupper(substr((string) $realNumber, -1)) == 'E') {
            $power = strtok('+-');
            ++$leadingSign;
        }

        $realNumber = substr((string) $workString, 0, strlen((string) $realNumber)+strlen((string) $power)+$leadingSign);

        if ($suffix != '') {
            $imaginary = substr((string) $workString, strlen((string) $realNumber));
    ...
    {
        if ($complexNumber[0] == '+') {
            $complexNumber = substr((string) $complexNumber, 1);
        }
    ...
    private static function nbrConversionFormat($xVal, $places)
    {
        if (!is_null($places)) {
            if (strlen((string) $xVal) <= $places) {
                return substr(str_pad((string) $xVal, $places, '0', STR_PAD_LEFT), -10);
            } else {
    ...
    public static function BINTODEC($x)
    {
        ...
        $x = (string) $x;
        if (strlen((string) $x) > preg_match_all('/[01]/', (string) $x, $out)) {
            return PHPExcel_Calculation_Functions::NaN();
        }
    ...
    public static function DECTOBIN($x, $places = null)
    {
        ...
        $x = (string) $x;
        if (strlen((string) $x) > preg_match_all('/[-0123456789.]/', (string) $x, $out)) {
            return PHPExcel_Calculation_Functions::VALUE();
        }
        $x = (string) floor($x);
        $r = decbin($x);
    ...
    public static function OCTTOBIN($x, $places = null)
    {
        ...
        $x = (string) $x;
        if (preg_match_all('/[01234567]/', (string) $x, $out) != strlen((string) $x)) {
            return PHPExcel_Calculation_Functions::NaN();
        }
        $r = decbin(octdec((string) $x));
    ...
    public static function OCTTOHEX($x, $places = null)
    {
        ...
        $hexVal = strtoupper(dechex(octdec((string) $x)));
