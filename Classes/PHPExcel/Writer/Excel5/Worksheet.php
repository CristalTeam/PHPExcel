...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
        ...
        foreach ($phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
            ...
            if (str_contains((string) $url, 'sheet://')) {
                // internal to current workbook
                $url = str_replace('sheet://', 'internal:', $url);

            } elseif (preg_match('/^(http:|https:|ftp:|mailto:)/', (string) $url)) {
                // URL
                // $url = $url;
    ...
    private function writeNumber($row, $col, $num, $xfIndex)
    {
        ...
        if (self::getByteOrder()) { // if it's Big Endian
            $xl_double = strrev((string) $xl_double);
        }
    ...
    private function writeLabel($row, $col, $str, $xfIndex)
    {
        ...
        if ($strlen > $this->xlsStringMaxLength) { // LABEL must be < 255 chars
            $str    = substr((string) $str, 0, $this->xlsStringMaxLength);
    ...
    {
        $note_length    = strlen((string) $note);
        ...
        $this->append($header . $data . substr((string) $note, 0, 2048));

        for ($i = $max_length; $i < $note_length; $i += $max_length) {
            $chunk  = substr((string) $note, $i, $max_length);
            $length = 0x0006 + strlen((string) $chunk);
            $header = pack("vv", $record, $length);
            $data   = pack("vvv", -1, 0, strlen((string) $chunk));
    ...
    public function writeUrlRange($row1, $col1, $row2, $col2, $url)
    {
        ...
        if (preg_match('[^internal:]', (string) $url)) {
            return($this->writeUrlInternal($row1, $col1, $row2, $col2, $url));
        }
        if (preg_match('[^external:]', (string) $url)) {
    ...
    public function writeUrlWeb($row1, $col1, $row2, $col2, $url)
    {
        ...
        $url         = implode("\0", preg_split("''", (string) $url, -1, PREG_SPLIT_NO_EMPTY));
        $url .= "\0\0\0";
    ...
    public function writeUrlInternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        // Strip URL type
        $url = preg_replace('/^internal:/', '', (string) $url);
    ...
    public function writeUrlExternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        //
        $url = preg_replace('/^external:/', '', (string) $url);
    ...
    private function writeRangeProtection()
    {
        foreach ($this->phpSheet->getProtectedCells() as $range => $password) {
            ...
            $recordData .= pack(
                'VV',
                0x0000,
                hexdec((string) $password)
            );

            $recordData .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong('p' . md5((string) $recordData));
    ...
    private function writeExternsheet($sheetname)
    {
        ...
        if ($this->phpSheet->getTitle() == $sheetname) {
            ...
            $length    = 0x02 + strlen((string) $sheetname);
            $cch       = strlen((string) $sheetname);
    ...
    private function writeSetup()
    {
        ...
        if (self::getByteOrder()) { // if it's Big Endian
            $numHdr = strrev((string) $numHdr);
            $numFtr = strrev((string) $numFtr);
        }
    ...
    private function writePassword()
    {
        ...
        $wPassword   = hexdec((string) $this->phpSheet->getProtection()->getPassword());     // Encoded password
    ...
    public function processBitmap($bitmap)
    {
        ...
        $identity = unpack("A2ident", (string) $data);
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                // explicit formula?
                if ($type == 0x03 && preg_match('/^\".*\"$/', (string) $dataValidation->getFormula1())) {
                    $options |= 0x01 << 7;
                }
