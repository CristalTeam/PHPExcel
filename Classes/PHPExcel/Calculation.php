...
class PHPExcel_Calculation
{
    ...
    private static function loadLocales()
    {
        ...
        foreach (glob($localeFileDirectory.'/*', GLOB_ONLYDIR) as $filename) {
            $filename = substr((string) $filename, strlen((string) $localeFileDirectory)+1);
            if ($filename != 'en') {
        ...
        if (in_array($language, self::$validLocaleLanguages)) {
            ...
            if ($locale != 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    [$localeFunction] = explode('##', (string) $localeFunction);    //    Strip out comments
                    if (str_contains((string) $localeFunction, '=')) {
                        [$fName, $lfName] = explode('=', (string) $localeFunction);
                        $fName = trim((string) $fName);
                        $lfName = trim((string) $lfName);
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        [$localeSetting] = explode('##', (string) $localeSetting);    //    Strip out comments
                        if (str_contains((string) $localeSetting, '=')) {
                            [$settingName, $settingValue] = explode('=', (string) $localeSetting);
                            $settingName = strtoupper(trim((string) $settingName));
                            switch ($settingName) {
                                case 'ARGUMENTSEPARATOR':
                                    self::$localeArgumentSeparator = trim((string) $settingValue);
                                    break;
                            }
                        }
                    }
        ...
        if (self::$localeLanguage !== 'en_us') {
            ...
            if (str_contains((string) $formula, '"')) {
                ...
                foreach ($temp as &$value) {
                    ...
                    if ($i = !$i) {
                        $value = preg_replace($from, (string) $to, (string) $value);
    ...
    public function _translateFormulaToLocale($formula)
    {
        if (self::$functionReplaceFromExcel === null) {
            ...
            foreach (array_keys(self::$localeBoolean) as $excelBoolean) {
                self::$functionReplaceFromExcel[] = '/(@?[^\w\.])'.preg_quote((string) $excelBoolean).'([^\w\.])/Ui';
            }
    ...
    public function _translateFormulaToEnglish($formula)
    {
        if (self::$functionReplaceFromLocale === null) {
            ...
            foreach (array_values(self::$localeBoolean) as $excelBoolean) {
                self::$functionReplaceFromLocale[] = '/(@?[^\w\.])'.preg_quote((string) $excelBoolean).'([^\w\.])/Ui';
            }
        }
    ...
    public static function wrapResult($value)
    {
        if (is_string($value)) {
            ...
            if (preg_match('/^'.self::CALCULATION_REGEXP_ERROR.'$/i', (string) $value, $match)) {
    ...
    {
        if (is_string($value)) {
            if ((isset($value[0])) && ($value[0] == '"') && (str_ends_with((string) $value, '"'))) {
    ...
    private function showValue($value)
    {
        if ($this->_debugLog->getWriteDebugLog()) {
            ...
            } elseif (is_string($value) && (trim((string) $value, '"') == $value)) {
                return '"'.$value.'"';
        ...
        if (str_contains((string) $formula, '{')) {
            ...
            if (str_contains((string) $formula, '"')) {
                ...
                foreach ($temp as &$value) {
                    //    Only count/replace in alternating array entries
                    if ($i = !$i) {
                        $openCount += substr_count((string) $value, '{');
                        $closeCount += substr_count((string) $value, '}');
...
//                echo 'Element with value '.$val.' is an Operand, Variable, Constant, String, Number, Cell Reference or Function<br />';
                ...
                if (preg_match('/^'.self::CALCULATION_REGEXP_FUNCTION.'$/i', (string) $val, $matches)) {
                    $val = preg_replace('/\s/u', '', (string) $val);
//                    echo 'Element '.$val.' is a Function<br />';
                    if (isset(self::$PHPExcelFunctions[strtoupper((string) $matches[1])]) || isset(self::$controlFunctions[strtoupper((string) $matches[1])])) {    // it's a function
                ...
                } elseif (preg_match('/^'.self::CALCULATION_REGEXP_CELLREF.'$/i', (string) $val, $matches)) {
...
//                    echo 'Element is a Variable, Constant, String, Number or Boolean<br />';
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if (str_contains('!', (string) $val)) {
                            [$rangeWS2, $val] = explode('!', (string) $val);
                        }
                        if ($rangeWS2 != '') {
                            $rangeWS2 .= '!';
                        }
                        if ((is_int($startRowColRef)) && (ctype_digit((string) $val)) &&
                            ($startRowColRef <= 1048576) && ($val <= 1048576)) {
                            //    Row range
                            $endRowColRef = ($pCellParent !== null) ? $pCellParent->getHighestColumn() : 'XFD';    //    Max 16,384 columns for Excel2007
                            $output[count($output)-1]['value'] = $rangeWS1.'A'.$startRowColRef;
                            $val = $rangeWS2.$endRowColRef.$val;
                        } elseif ((ctype_alpha((string) $startRowColRef)) && (ctype_alpha((string) $val)) &&
                            (strlen((string) $startRowColRef) <= 3) && (strlen((string) $val) <= 3)) {
                            //    Column range
...
//        }
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        if (PHPExcel_Calculation_Functions::getCompatibilityMode() != PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
            if (is_string($operand1)) {
                $operand1 = strtoupper((string) $operand1);
            }
            if (is_string($operand2)) {
                $operand2 = strtoupper((string) $operand2);
            }
        }
    ...
    private function strcmpLowercaseFirst($str1, $str2)
    {
        ...
        return strcmp((string) $inversedStr1, (string) $inversedStr2);
    }
...
//            echo '<br />';
    ...
    {
        $pFunction = strtoupper((string) $pFunction);
